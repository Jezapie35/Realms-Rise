...
import { availableUpgrades, isUpgradeAvailable } from "@/data/gameEngine";
          ...
          list.map((u) => {
            ...
            return (
              <Pressable
                ...
                testID={`upgrade-${u.id}`}
              >
                <IconPlaceholder size={36} tint={rColor} label={"T" + upgradeTierImage(u.tier)} borderRadius={4} />
      ...
      >
        {selected && (
          <UpgradeTooltip
            ...
            available={isUpgradeAvailable(selected.id, state)}
            onBuy={() => {
