...
import SkillCommerce from "./icons/ui/SkillCommerce";
import SkillMilitary from "./icons/ui/SkillMilitary";
import SkillFaith from "./icons/ui/SkillFaith";
import SkillLineage from "./icons/ui/SkillLineage";
import SkillCross from "./icons/ui/SkillCross";
import SkillPinnacle from "./icons/ui/SkillPinnacle";

const BRANCH_ICON_MAP: Record<string, React.FC<{ size?: number }>> = {
  commerce: SkillCommerce,
  military: SkillMilitary,
  faith: SkillFaith,
  lineage: SkillLineage,
  cross: SkillCross,
  pinnacle: SkillPinnacle,
};

interface Props {
  visible: boolean;
  onClose: () => void;
  /** Cookie Clicker-style ascension shop after declaring sovereignty */
  legacyShopMode?: boolean;
  onStartKingdom?: () => void;
}
...
export default function SkillTreeSheet({
  visible,
  onClose,
  legacyShopMode = false,
  onStartKingdom,
}: Props) {
  ...
  return (
    <Modal
      visible={visible}
      animationType="slide"
      onRequestClose={legacyShopMode ? undefined : onClose}
    >
      <View style={[styles.root, { paddingTop: topPad }]}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{legacyShopMode ? "Legacy Hall" : "Skill Tree"}</Text>
          <View style={styles.sealsRow}>
            <ExpoImage source={require('@/assets/images/seal.png')} style={styles.sealImg} contentFit="contain" transition={0} cachePolicy="memory-disk" />
        ...
        {legacyShopMode && (
          <View style={styles.legacyBanner}>
            <Text style={styles.legacyBannerTitle}>Spend your Royal Seals</Text>
            <Text style={styles.legacyBannerSub}>
              Your legacy upgrades apply when you begin the new kingdom.
            </Text>
          </View>
        )}
        ...
        {legacyShopMode && onStartKingdom && (
          <View style={styles.startKingdomWrap}>
            <Pressable
              onPress={onStartKingdom}
              style={({ pressed }) => [styles.startKingdomBtn, pressed && { opacity: 0.85 }]}
              testID="start-new-kingdom"
            >
              <Text style={styles.startKingdomText}>Begin New Kingdom</Text>
              <Text style={styles.startKingdomSub}>
                {state.sealsAvailable > 0
                  ? `${formatGold(state.sealsAvailable)} seal${state.sealsAvailable === 1 ? "" : "s"} unspent — you can keep shopping or start now`
                  : "Start your next run with all legacy powers active"}
              </Text>
            </Pressable>
          </View>
        )}
...
function SkillNodeCard({ node, color, unlocked, reqMet, affordable, onUnlock }: NodeProps) {
  ...
  return (
    ...
    >
      <View style={[styles.iconWrap, { borderColor: color }]}>
        {(() => { const Icon = BRANCH_ICON_MAP[node.branch]; return Icon ? <Icon size={28} /> : null; })()}
      </View>
      <View style={styles.cardBody}>
        <View style={styles.cardTitleRow}>
...
const styles = StyleSheet.create({
  ...
  legacyBanner: {
    ...
    backgroundColor: COLORS.bg3,
    borderBottomWidth: 1,
    borderBottomColor: COLORS.gold4,
  },
  legacyBannerTitle: {
    color: COLORS.textGold,
    fontFamily: FONTS.serif,
