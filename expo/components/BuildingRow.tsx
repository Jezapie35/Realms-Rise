...
import { Building, BUILDING_TINTS } from "@/data/buildings";
  ...
  const eachGps = useMemo<number>(
    () =>
      count > 0
        ...
        : 0,
    [building.id, purchasedUpgrades, unlockedSkillNodes, activeBonus, runStartTime, count, now],
  );
  ...
  return (
    <View style={[styles.row, altRow && styles.rowAlt]}>
      <View style={styles.imageZone}>
        <IconPlaceholder
          size={62}
          tint={BUILDING_TINTS[building.id] ?? COLORS.gold4}
          label={building.name.split(" ")[0]}
          borderRadius={10}
        />
        {count > 0 && (
      ...
      >
        {buyMode !== "1" && (
          <Text style={[styles.buyQty, !affordable && styles.buyDim]}>×{quantity}</Text>
        )}
