...
export default function MainScreen() {
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]}>
      ...
      {/* Floating skill tree button — unlocks after first prestige */}
      {state.prestigeCount > 0 && (
        <Pressable
          onPress={() => setShowSkillTree(true)}
          style={styles.skillBtn}
          testID="skill-tree-btn"
        >
          <IconPlaceholder size={36} tint={COLORS.red} borderRadius={18} />
          {state.sealsAvailable > 0 && (
            <View style={styles.skillBadge}>
              <Text style={styles.skillBadgeText}>{formatGold(state.sealsAvailable)}</Text>
            </View>
          )}
        </Pressable>
      )}
  ...
  return (
    <Animated.View style={[styles.prestigeWrap, { borderWidth: border }]}>
      ...
      <Pressable onPress={onPress} style={styles.prestigeInner} testID="prestige-btn">
        <IconPlaceholder size={48} tint={COLORS.red} borderRadius={24} />
