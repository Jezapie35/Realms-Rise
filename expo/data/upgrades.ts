...
const BUILDING_UPGRADE_NAMES: Record<string, string[]> = {
  ...
  arcane_library: [
    "Forbidden Texts", "Runic Codex", "Mystical Compendium", "Arcane Archives",
    "Tome of Ages", "Grimoire Vaults", "Leyline Index", "Scrolls of Power",
    "Library of Eternity", "The Infinite Shelf", "Omniscient Codex", "Library Eternal",
  ],
  wizards_tower: [
    "Mana Crystals", "Apprentice Mages", "Spell Matrices", "Arcane Conduits",
    "Leyline Tap", "Wizard's Guild", "Astral Staff", "Tower of Wonders",
    "Grand Spire", "Cosmic Wand", "Staff of Eternity", "Tower Eternal",
  ],
  dragon_lair: [
    "Hoard Expansion", "Fire Tribute", "Dragon Riders", "Scale Armour Trade",
    "Dragon Flight", "Brood Collection", "Ancient Lineage", "Dragon Compact",
    "Sky Terror", "Elder Dragon", "Wyrmhoard", "Lair Eternal",
  ],
  astral_observatory: [
    "Celestial Charts", "Star Maps", "Cosmic Lens", "Orbital Surveys",
    "Void Telescope", "Constellation Index", "Nebula Readings", "Galactic Charts",
    "Star Contracts", "Void Markets", "Astral Mandate", "Observatory Eternal",
  ],
  realm_gate: [
    "Dimensional Toll", "Planar Tariffs", "Gate Tax", "Void Commerce",
    "Realm Expansion", "Planar Empire", "Dimensional Monopoly", "Rift Markets",
    "Interdimensional Trade", "Void Sovereignty", "Planar Eternity", "Gate Eternal",
  ],
};
...
const SYNERGY_UPGRADES: Upgrade[] = [
  ...
  // ── New building synergies ─────────────────────────────────────────────────
  { id: "syn_arcane_cath", buildingId: "_synergy", tier: 0, rarity: "epic", name: "Sacred Knowledge", description: "Arcane Libraries and Cathedrals each ×2.", cost: 8_000_000_000, requires: { buildingCounts: { arcane_library: 10, cathedral: 25 } }, effects: [{ kind: "synergy_mult", buildings: ["arcane_library", "cathedral"], multiplier: 2 }] },
  { id: "syn_wizard_arcane", buildingId: "_synergy", tier: 0, rarity: "epic", name: "Tower Scholarship", description: "Wizard's Towers ×3. Arcane Libraries ×2.", cost: 60_000_000_000, requires: { buildingCounts: { wizards_tower: 10, arcane_library: 25 } }, effects: [{ kind: "synergy_mult", buildings: ["wizards_tower"], multiplier: 3 }, { kind: "synergy_mult", buildings: ["arcane_library"], multiplier: 2 }] },
  { id: "syn_dragon_barracks", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Dragonguard", description: "Dragon Lairs and Barracks each ×2. Click power ×2.", cost: 500_000_000_000, requires: { buildingCounts: { dragon_lair: 10, barracks: 50 } }, effects: [{ kind: "synergy_mult", buildings: ["dragon_lair", "barracks"], multiplier: 2 }, { kind: "click_mult", multiplier: 2 }] },
  { id: "syn_astral_wizard", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Celestial Spellwork", description: "Astral Observatories and Wizard's Towers each ×3.", cost: 4_000_000_000_000, requires: { buildingCounts: { astral_observatory: 10, wizards_tower: 25 } }, effects: [{ kind: "synergy_mult", buildings: ["astral_observatory", "wizards_tower"], multiplier: 3 }] },
  { id: "syn_gate_all", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Multiplanar Economy", description: "Realm Gates ×2. All production ×1.25.", cost: 50_000_000_000_000, requires: { buildingCounts: { realm_gate: 10, palace: 25 } }, effects: [{ kind: "synergy_mult", buildings: ["realm_gate"], multiplier: 2 }, { kind: "global_gps_mult", multiplier: 1.25 }] },
  // 100-milestone synergies for new buildings
  { id: "syn_100_arcane", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Grand Archive", description: "Arcane Libraries ×4. All production ×1.1.", cost: 100_000_000_000, requires: { buildingCounts: { arcane_library: 100 } }, effects: [{ kind: "synergy_mult", buildings: ["arcane_library"], multiplier: 4 }, { kind: "global_gps_mult", multiplier: 1.1 }] },
  { id: "syn_100_wizard", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Order of Mages", description: "Wizard's Towers ×4. Click power ×2.", cost: 700_000_000_000, requires: { buildingCounts: { wizards_tower: 100 } }, effects: [{ kind: "synergy_mult", buildings: ["wizards_tower"], multiplier: 4 }, { kind: "click_mult", multiplier: 2 }] },
  { id: "syn_100_dragon", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Dragon Empire", description: "Dragon Lairs ×5. Barracks ×2.", cost: 5_000_000_000_000, requires: { buildingCounts: { dragon_lair: 100 } }, effects: [{ kind: "synergy_mult", buildings: ["dragon_lair"], multiplier: 5 }, { kind: "synergy_mult", buildings: ["barracks"], multiplier: 2 }] },
  { id: "syn_100_astral", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Star Sovereignty", description: "Astral Observatories ×4. All production ×1.15.", cost: 40_000_000_000_000, requires: { buildingCounts: { astral_observatory: 100 } }, effects: [{ kind: "synergy_mult", buildings: ["astral_observatory"], multiplier: 4 }, { kind: "global_gps_mult", multiplier: 1.15 }] },
  { id: "syn_100_gate", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Dimensional Dominion", description: "Realm Gates ×5. All production ×1.2.", cost: 400_000_000_000_000, requires: { buildingCounts: { realm_gate: 100 } }, effects: [{ kind: "synergy_mult", buildings: ["realm_gate"], multiplier: 5 }, { kind: "global_gps_mult", multiplier: 1.2 }] },
  // Grand synergy for all 15 buildings
  { id: "syn_grand_all15_100", buildingId: "_synergy", tier: 0, rarity: "legendary", name: "Planar Unity", description: "All production ×1.5. Click power ×5.", cost: 5_000_000_000_000_000, requires: { buildingCounts: { peasant_hut: 100, market_stall: 100, blacksmith: 100, tavern: 100, mill: 100, barracks: 100, cathedral: 100, castle_tower: 100, royal_treasury: 100, palace: 50, arcane_library: 50, wizards_tower: 25, dragon_lair: 10, astral_observatory: 5, realm_gate: 1 } }, effects: [{ kind: "global_gps_mult", multiplier: 1.5 }, { kind: "click_mult", multiplier: 5 }] },
  ...
  ...((): Upgrade[] => {
    ...
    const flavours: Record<string, string[]> = {
      ...
      arcane_library: ["Arcane Hegemony", "Tome Ascendant", "Lore Apotheosis", "Grimoire Cosmos", "Leyline Zenith", "Eternal Archive", "Arcane Infinity", "Codex Cosmos"],
      wizards_tower: ["Spire Hegemony", "Mage Ascendant", "Sorcery Apotheosis", "Spell Cosmos", "Tower Zenith", "Eternal Spellwork", "Magic Infinity", "Arcane Spire Cosmos"],
      dragon_lair: ["Flame Hegemony", "Wyrm Ascendant", "Dragon Apotheosis", "Scale Cosmos", "Hoard Zenith", "Eternal Dragonfire", "Wyrm Infinity", "Dragon Cosmos"],
      astral_observatory: ["Star Hegemony", "Void Ascendant", "Stellar Apotheosis", "Nebula Cosmos", "Astral Zenith", "Eternal Starfield", "Cosmic Infinity", "Celestial Cosmos"],
      realm_gate: ["Gate Hegemony", "Rift Ascendant", "Planar Apotheosis", "Void Cosmos", "Dimensional Zenith", "Eternal Gateway", "Rift Infinity", "Planar Cosmos"],
    };
    for (const b of BUILDINGS) {
